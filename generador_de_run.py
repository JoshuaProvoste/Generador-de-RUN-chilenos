...
#Versión original de Sebastián Zapata en https://code.sololearn.com/cwcztuOkXTCg/#py
#Modificado por @JoshuaProvoste
...
for i in range(999, 0, -1):
   ...
   for x in range(0,999):
      if len(str(x)) == 1:
         tur = "20."+str(i)+".00"+str(x)
         rut = int(tur.replace(".",""))
         if rut != 0:
            time.sleep(0.5)
            rut_valido = str(rut)+"-"+digitoVerificador(rut)
            rut_txt = open("bloque_de_run_20.txt","a")
            rut_txt.write(rut_valido+"\n")
            rut_txt.close()
      else:
            pass